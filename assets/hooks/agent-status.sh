#!/bin/sh
# zym agent-status reporter — invoked by Claude Code hooks (see AgentTerminal).
#
# Writes a one-word status (idle|working|waiting) to $ZYM_STATUS_FILE, which the
...
# hook) appends the edited file's path to $ZYM_STATUS_FILE.files instead. Every
# payload also carries the claude session id, captured to $ZYM_STATUS_FILE.session
...
if [ -n "$sid" ]; then
  printf '%s' "$sid" > "$ZYM_STATUS_FILE.session.tmp" 2>/dev/null &&
    mv "$ZYM_STATUS_FILE.session.tmp" "$ZYM_STATUS_FILE.session" 2>/dev/null
fi
...
# next hook, so every payload that carries it refreshes $ZYM_STATUS_FILE.mode.
mode=$(printf '%s' "$payload" | sed -n 's/.*"permission_mode":"\([^"]*\)".*/\1/p' | head -1)
if [ -n "$mode" ]; then
  printf '%s' "$mode" > "$ZYM_STATUS_FILE.mode.tmp" 2>/dev/null &&
    mv "$ZYM_STATUS_FILE.mode.tmp" "$ZYM_STATUS_FILE.mode" 2>/dev/null
fi

status="$1"

# PostToolUse(Edit/Write/…) records the touched file for change-awareness: append
# its path to $ZYM_STATUS_FILE.files (one per line), which the editor watches.
if [ "$status" = "files" ]; then
  fp=$(printf '%s' "$payload" | sed -n 's/.*"file_path":"\([^"]*\)".*/\1/p' | head -1)
  [ -n "$fp" ] && printf '%s\n' "$fp" >> "$ZYM_STATUS_FILE.files"
  exit 0
fi
...
# $ZYM_STATUS_FILE.wtcreate; the editor warns if no set_worktree follows.
if [ "$status" = "bash" ]; then
  ...
  case "$cmd" in
    *"git worktree add"*)
      ...
      if [ -n "$wt" ]; then
        printf '%s' "$wt" > "$ZYM_STATUS_FILE.wtcreate.tmp" 2>/dev/null &&
          mv "$ZYM_STATUS_FILE.wtcreate.tmp" "$ZYM_STATUS_FILE.wtcreate" 2>/dev/null
      fi
      ;;
  esac
  exit 0
...
printf '%s' "$status" > "$ZYM_STATUS_FILE.tmp" 2>/dev/null &&
  mv "$ZYM_STATUS_FILE.tmp" "$ZYM_STATUS_FILE" 2>/dev/null
exit 0
