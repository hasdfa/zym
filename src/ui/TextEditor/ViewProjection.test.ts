...
/** A resolver over a fixed set of in-memory sources (sourceKey → full text). */
function resolverFor(sources: Record<string, string>) {
  const rows: Record<string, string[]> = {};
  for (const [key, text] of Object.entries(sources)) rows[key] = rowsOf(text);
  return {
    rows,
    resolve: (seg: { sourceKey: string; startRow: number; endRow: number }) =>
      rows[seg.sourceKey].slice(seg.startRow, seg.endRow + 1),
  };
}

/** One full-file editable segment (the normal-editor degenerate case). */
function fileItem(sourceKey: string, lastRow: number): Item {
  return { type: 'segment', segment: { sourceKey, startRow: 0, endRow: lastRow, editable: true, kind: 'real' } };
}
...
test('single full-file segment is the identity path', () => {
  ...
  assert.equal(p.projectionText, text, 'projection round-trips the file exactly');
  assert.equal(p.viewText, text);
  assert.equal(p.viewRowCount, 3); // "const x = 1;", "const y = 2;", ""

  // source ↔ view is identity (row + codepoint column pass through).
  assert.deepEqual(p.viewToSource(0, 6), { kind: 'source', sourceKey: 'f.ts', row: 0, column: 6, segmentIndex: 0 });
  assert.deepEqual(p.sourceToView('f.ts', 1, 0), { row: 1, column: 0 });
  assert.equal(p.viewRowForSource('f.ts', 1), 1);
  assert.equal(p.isViewPositionEditable(1, 0), true);
  assert.equal(p.isViewRangeEditable(0, 2), true);
});

test('identity projection rejects a foreign source key', () => {
  const { resolve, rows } = resolverFor({ 'f.ts': 'a\nb\n' });
  const p = ViewProjection.build([fileItem('f.ts', rows['f.ts'].length - 1)], resolve);
  assert.equal(p.sourceToView('other.ts', 0, 0), null);
});
...
test('multi-source projection maps each row to its source (and blocks)', () => {
  ...
  const items: Item[] = [
    ...
    { type: 'segment', segment: { sourceKey: 'a.ts', startRow: 1, endRow: 2, editable: false, kind: 'real' } },
    { type: 'block', block: { kind: 'blank', text: '' } },
    { type: 'block', block: { kind: 'header', text: 'b.ts' } },
    { type: 'segment', segment: { sourceKey: 'b.ts', startRow: 0, endRow: 1, editable: false, kind: 'real' } },
  ];
  ...
  assert.equal(p.projectionRowCount, 7);
  assert.equal(p.projectionText, 'a.ts\nconst aaa = 1;\nfunction fa() {}\n\nb.ts\nconst bbb = 2;\nlet ccc = 3;');

  assert.deepEqual(p.viewToSource(0, 0), { kind: 'block', block: 'header' });
  assert.deepEqual(p.viewToSource(3, 0), { kind: 'block', block: 'blank' });
  assert.equal(p.viewToSource(1, 0).kind, 'source');
  assert.deepEqual(p.sourceRowAtViewRow(2), { sourceKey: 'a.ts', sourceRow: 2 });
  assert.deepEqual(p.sourceRowAtViewRow(6), { sourceKey: 'b.ts', sourceRow: 1 });
  assert.equal(p.sourceRowAtViewRow(4), null, 'a header row has no source');

  // The B excerpt's source rows 0,1 translate to view rows 5,6 — the coordinate map at work.
  assert.equal(p.viewRowForSource('b.ts', 0), 5);
  assert.equal(p.viewRowForSource('b.ts', 1), 6);
  assert.equal(p.projectionRowForSource('a.ts', 2), 2);

  assert.deepEqual(p.blockRows(), [
    { viewRow: 0, kind: 'header' },
    { viewRow: 3, kind: 'blank' },
    { viewRow: 4, kind: 'header' },
  ]);
});
...
test('a fold collapses projection rows into a placeholder and translates around it', () => {
  ...
  assert.equal(p.viewRowCount, 4); // L0, [3], L4, ""

  // L4 (source row 4) is shown on view row 2; round-trips both ways.
  assert.deepEqual(p.viewToSource(2, 0), { kind: 'source', sourceKey: 'f', row: 4, column: 0, segmentIndex: 0 });
  assert.deepEqual(p.sourceToView('f', 4, 0), { row: 2, column: 0 });

  // The placeholder row is "inside a fold" — no live source position.
  assert.deepEqual(p.viewToSource(1, 0), { kind: 'fold' });
  assert.deepEqual(p.viewToSource(1, 1), { kind: 'fold' });

  // A folded source row (L2) maps onto the placeholder's start.
  assert.deepEqual(p.sourceToView('f', 2, 0), { row: 1, column: 0 });

  // Rows above the fold are untouched.
  assert.deepEqual(p.viewToSource(0, 1), { kind: 'source', sourceKey: 'f', row: 0, column: 1, segmentIndex: 0 });
...
test('fold composes with the multi-source map', () => {
  ...
  assert.deepEqual(p.viewToSource(5, 0), { kind: 'fold' });
  // The A excerpt above the fold is unaffected.
  assert.deepEqual(p.sourceRowAtViewRow(1), { sourceKey: 'a.ts', sourceRow: 1 });
  // A folded B source row maps onto the placeholder row.
  assert.deepEqual(p.sourceToView('b.ts', 0, 0), { row: 5, column: 0 });
...
test('editability gates blocks, phantoms, cross-source ranges, and folds', () => {
  ...
  const items: Item[] = [
    ...
    { type: 'segment', segment: { sourceKey: 'f1', startRow: 0, endRow: 1, editable: true, kind: 'real' } },
    // a diff "removed" row: real text from the base blob, but read-only.
    { type: 'segment', segment: { sourceKey: 'base', startRow: 0, endRow: 0, editable: false, kind: 'phantom' } },
    { type: 'segment', segment: { sourceKey: 'f2', startRow: 0, endRow: 1, editable: true, kind: 'real' } },
  ];
  const p = ViewProjection.build(items, resolve);
  // rows: 0:f1(header) 1:a 2:b 3:old(phantom) 4:c 5:d

  assert.equal(p.isViewPositionEditable(0, 0), false, 'header block is not editable');
  assert.equal(p.isViewPositionEditable(1, 0), true, 'f1 body is editable');
  assert.equal(p.isViewPositionEditable(3, 0), false, 'phantom removed row is not editable');

  assert.equal(p.isViewRangeEditable(1, 2), true, 'both f1 rows, one source');
  assert.equal(p.isViewRangeEditable(1, 3), false, 'range hits the phantom row');
  assert.equal(p.isViewRangeEditable(4, 5), true, 'both f2 rows, one source');
  assert.equal(p.isViewRangeEditable(2, 4), false, 'range crosses phantom + sources');
});
...
test('an adjacent two-source range is not editable (would span sources)', () => {
  ...
  assert.equal(p.isViewRangeEditable(0, 1), true, 'within f1');
  assert.equal(p.isViewRangeEditable(2, 3), true, 'within f2');
  assert.equal(p.isViewRangeEditable(1, 2), false, 'spans f1 → f2');
});
...
test('fold offsets are codepoint-accurate across astral characters', () => {
  ...
  const items: Item[] = [
    ...
    { type: 'segment', segment: { sourceKey: 's', startRow: 0, endRow: 2, editable: true, kind: 'real' } },
  ];
  const p = ViewProjection.build(items, resolve);
  ...
  assert.deepEqual(p.viewToSource(1, 3), { kind: 'source', sourceKey: 's', row: 0, column: 3, segmentIndex: 0 });
  ...
  p.addFold(2, 5, '·');
  assert.equal(p.viewText, 'h\n·\nz\n');
  assert.deepEqual(p.sourceToView('s', 1, 0), { row: 2, column: 0 }, '"z" stays on its row past the astral fold');
  assert.deepEqual(p.viewToSource(1, 0), { kind: 'fold' });
});
